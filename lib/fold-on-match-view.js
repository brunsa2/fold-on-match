...
export default class FoldOnMatchView {
  ...
  render () {
    return (
      <div tabindex="-1" class="fold-on-match">
        <header class="header">
          <span ref="closeButton" class="header-item close-button pull-right">
            <i class="icon icon-x clickable"></i>
          </span>

          <span ref="descriptionLabel" class="header-item description">
            Find and Fold in Current Buffer
          </span>

          <span class="header-item options-label pull-right">
            <span>Finding with Options: </span>
            <span ref="optionsLabel" class="options">Case Insensitive</span>
            <span class="btn-group btn-toggle btn-group-options">
              <button ref="regexOptionButton" class="btn">
                <svg class="icon" innerHTML="<use href='#fold-on-match-icon-regex' />" />
              </button>
              <button ref="caseOptionButton" class="btn">
                <svg class="icon" innerHTML="<use href='#fold-on-match-icon-case' />" />
              </button>
              <button ref="selectionOptionButton" class="btn">
                <svg class="icon" innerHTML="<use href='#fold-on-match-icon-selection' />" />
              </button>
              <button ref="wholeWordButton" class="btn">
                <svg class="icon" innerHTML="<use href='#fold-on-match-icon-word' />" />
              </button>
            </span>
          </span>
        </header>
        ...
        <section class="input-block find-container">
          <div class="input-block-item input-block-item--flex editor-container">
            ...
            <div class="find-meta-container">
              <span ref="resultCounter" class="text-subtle result-counter" />
            </div>

            <div class="input-block-item">
              <div class="btn-group btn-group-find">
                <button ref="nextButton" class="btn btn-next">Find</button>
              </div>
              <div class="btn-group btn-group-fold-all">
                <button ref="foldAllButton" class="btn btn-next">Fold All</button>
              </div>
              <div class="btn-group btn-group-unfold-all">
                <button ref="unfoldAllButton" class="btn btn-next">Unfold All</button>
              </div>
            </div>
          </div>
        </section>
        ...
        {
          etch.dom.svg({style: {display: 'none'}, innerHTML: `
            <symbol id="fold-on-match-icon-regex" viewBox="0 0 20 16" stroke="none" fill-rule="evenodd">
            ...
            <symbol id="fold-on-match-icon-case" viewBox="0 0 20 16" stroke="none" fill-rule="evenodd">
            ...
            <symbol id="fold-on-match-icon-selection" viewBox="0 0 20 16" stroke="none" fill-rule="evenodd">
            ...
            <symbol id="fold-on-match-icon-word" viewBox="0 0 20 16" stroke="none" fill-rule="evenodd">
